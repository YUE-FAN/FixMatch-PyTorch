...
class RandomCrop(object):
    ...
    def __call__(self, pic):
        ...
        if len(pic.shape) == 2:
            ...
            raise Exception('image must grey or 3-channels')
...
class Normalize_11(object):
    """
    Normalize the values of a torch Tensor between -1 and 1
    """
    def __call__(self, pic):
        """
        :param pic: torch Tensor
        :return: same torch Tensor with its values normalized between -1 and 1
        """
        # check type of [pic]
        if not isinstance(pic, torch.Tensor):
            raise TypeError('img should be torch Tensor. Got {}'.format(type(pic)))
        pic = pic * (2.0 / 255) - 1.0
        return pic
...
class NumpyToTensor(object):
    ...
    def __call__(self, pic):
        ...
        return torch.from_numpy(pic.transpose((2, 0, 1)).copy()).float()
...
class PILToNumpy(object):
    def __call__(self, pic):
        ...
        if pic.mode == 'RGB':
            ...
            raise Exception('Image should be RGB or RGBA!!!')
...
class Pad(object):
    ...
    def __call__(self, pic):
        ...
        if len(pic.shape) == 3:
            pic = np.pad(pic, ((pad_top, pad_bottom), (pad_left, pad_right), (0, 0)), self.padding_mode)
        # Grayscale image
        if len(pic.shape) == 2:
            pic = np.pad(pic, ((pad_top, pad_bottom), (pad_left, pad_right)), self.padding_mode)
